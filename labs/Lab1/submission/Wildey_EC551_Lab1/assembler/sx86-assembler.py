...
if __name__ == "__main__":
	if len(sys.argv) < 2:
		print "Not enough arguments.  Did you include the filename?"
		sys.exit()
	else:
		pass
...
#        print "Assembling:", " ".join(fields)
	...
	outf.write( "".join([("%04x\n"%item) for item in machinecode]) )
	print( "".join([("%04x"%item) for item in machinecode]) )
	# outf.write( "".join([("%04x\n"%0) for i in range(curr_addr, 4096)]) )
	outf.write( "".join([("%04x\n"%0) for i in range(curr_addr, 61)]) )
	outf.close()
